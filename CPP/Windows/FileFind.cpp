...
bool CFindFile::Close()
{
  ...
  if (_handle != INVALID_HANDLE_VALUE) {
    if ((rc = DosFindClose(_handle)) != NO_ERROR) {
      errno = rc;
      return false;
    }
  }
...
bool CFindFile::FindFirst(CFSTR wildcard, CFileInfo &fi)
{
  if (!Close())
    return false;
  #ifdef __OS2__
    FILEFINDBUF3L ffbuf3;
    ULONG cNames = 1;
    APIRET rc = DosFindFirst(wildcard, &_handle, FILE_DIRECTORY | FILE_SYSTEM | FILE_HIDDEN,
                             &ffbuf3, sizeof(ffbuf3), &cNames, FIL_STANDARDL);
    if(rc != NO_ERROR) {
      errno = rc;
      return false;
    }
    NTime::DosTimeToFileTime(DOSTIMEFROMOS2(ffbuf3.fdateCreation, ffbuf3.ftimeCreation), fi.CTime);
    NTime::DosTimeToFileTime(DOSTIMEFROMOS2(ffbuf3.fdateLastAccess, ffbuf3.ftimeLastAccess), fi.ATime);
    NTime::DosTimeToFileTime(DOSTIMEFROMOS2(ffbuf3.fdateLastWrite, ffbuf3.ftimeLastWrite), fi.MTime);
    fi.Attrib = ffbuf3.attrFile;
    fi.Size = ffbuf3.cbFile;
    fi.IsDevice = false;
    fi.Name = ffbuf3.achName;
    return true;
  #else
    #ifndef _UNICODE
    if (!g_IsNT)
...
bool CFindFile::FindNext(CFileInfo &fi)
{
    ...
    ULONG cNames = 1;
    APIRET rc;
    if((rc = DosFindNext(_handle, &ffbuf3, sizeof(ffbuf3), &cNames)) != NO_ERROR) {
      errno = rc;
      return false;
    }
    NTime::DosTimeToFileTime(DOSTIMEFROMOS2(ffbuf3.fdateCreation, ffbuf3.ftimeCreation), fi.CTime);
