...
void GetCurUtcFileTime(FILETIME &ft)
{
  ...
  if(GetSecondsSince1601(os2time.year, os2time.month, os2time.day,
                         os2time.hours, os2time.minutes, os2time.seconds, wintime))
  {
    wintime *= kNumTimeQuantumsInSecond;
...
#ifdef __OS2__
BOOL LocalFileTimeToFileTime(const FILETIME* lt, FILETIME* ft)
{
  UInt64 wintime = (((UInt64)lt->dwHighDateTime) << 32) + lt->dwLowDateTime;
  struct timeb tmb;

  ftime(&tmb);
  wintime += (UInt64)(tmb.timezone - (tmb.dstflag ? 60 : 0)) * 60 * NWindows::NTime::kNumTimeQuantumsInSecond;
  ft->dwLowDateTime  = (UInt32)wintime;
  ft->dwHighDateTime = (UInt32)(wintime >> 32);
  return TRUE;
}

BOOL FileTimeToLocalFileTime(const FILETIME* ft, FILETIME* lt)
{
  UInt64 wintime = (((UInt64)ft->dwHighDateTime) << 32) + ft->dwLowDateTime;
  struct timeb tmb;
  ...
  ftime(&tmb);
  wintime -= (UInt64)(tmb.timezone - (tmb.dstflag ? 60 : 0)) * 60 * NWindows::NTime::kNumTimeQuantumsInSecond;
  lt->dwLowDateTime  = (UInt32)wintime;
  lt->dwHighDateTime = (UInt32)(wintime >> 32);
  return TRUE;
}
#endif
