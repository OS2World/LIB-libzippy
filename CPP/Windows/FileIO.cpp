    ...
    DWORD shareMode, DWORD creationDisposition, DWORD flagsAndAttributes)
{
  if (!Close())
  ...
  #ifdef __OS2__
    ULONG ulAction;
    ULONG fsOpenMode = OPEN_FLAGS_FAIL_ON_ERROR;
    ...
    } else if(shareMode & FILE_SHARE_READ) {
    ...
    APIRET rc = DosOpenL(fileName, &_handle, &ulAction, 0, flagsAndAttributes,
                         creationDisposition, fsOpenMode, 0);
    if(rc != NO_ERROR) {
...
bool CFileBase::Close()
{
  ...
  #ifdef __OS2__
  APIRET rc;
  if ((rc = DosClose(_handle)) != NO_ERROR) {
    errno = rc;
    return false;
  }
  #else
...
bool CFileBase::GetLength(UInt64 &length) const
{
    ...
    if ((rc = DosSetFilePtrL(_handle, 0, FILE_CURRENT, &pos)) == NO_ERROR) {
      if ((rc = DosSetFilePtrL(_handle, 0, FILE_END, (PLONGLONG)&length)) == NO_ERROR) {
       rc = DosSetFilePtrL(_handle, pos, FILE_BEGIN, &pos);
      }
    }
    if (rc == NO_ERROR) {
      return true;
...
{
  #ifdef __OS2__
    APIRET rc = DosSetFilePtrL(_handle, distanceToMove, moveMethod, (PLONGLONG)&newPosition);
    if (rc == NO_ERROR) {
      return true;
    }
    errno = rc;
...
{
  #ifdef __OS2__
    APIRET rc = DosRead(_handle, data, size, (PULONG)&processedSize);
    if (rc == NO_ERROR) {
      return true;
    }
    errno = rc;
...
bool COutFile::SetTime(const FILETIME *cTime, const FILETIME *aTime, const FILETIME *mTime)
{
    ...
    if ((rc = DosQueryFileInfo(_handle, FIL_STANDARD, &fsts3PathInfo, ulBufferSize)) == NO_ERROR)
    {
      ...
      if ((rc = DosSetFileInfo(_handle, FIL_STANDARD, &fsts3PathInfo, ulBufferSize)) == NO_ERROR) {
        return true;
      }
    }
    errno = rc;
...
bool COutFile::WritePart(const void *data, UInt32 size, UInt32 &processedSize)
{
  ...
  DWORD processedLoc = 0;
  #ifdef __OS2__
  bool res = ((errno = DosWrite(_handle, (PVOID)data, size, (PULONG)&processedLoc)) == NO_ERROR);
  #else
...
bool COutFile::SetEndOfFile()
{
    ...
    APIRET rc;
    if((rc = DosSetFilePtrL(_handle, 0, FILE_CURRENT, &pos)) == NO_ERROR) {
      rc = DosSetFileSizeL(_handle, pos);
    }
    if (rc == NO_ERROR) {
      return true;
    }
