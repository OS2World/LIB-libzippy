...
void CInArchive::ReadHeaderReal(CItemEx &item)
{
  ...
  #ifdef __OS2__
  if(item.Type == NHeader::NBlockType::kFileHeader) {
  #endif
...
HRESULT CInArchive::GetNextItem(CItemEx &item, ICryptoGetTextPassword *getTextPassword, bool &decryptionError, AString &errorMessage)
{
  ...
  for (;;)
  {
        ...
        )
    {
      ...
      #ifdef __OS2__
      if (strcmp(item.Name, kSubHeadTypeOS2EA) == 0) {
        SeekInArchive(item.GetDataPosition());
        item.SubBlockData.SetCapacity(item.PackSize);
        size_t resSize = item.PackSize;
        if (ReadStream(m_Stream, (Byte*)item.SubBlockData, &resSize) != S_OK ||
            resSize != item.PackSize)
        {
          errorMessage = k_UnexpectedEnd;
          return S_FALSE;
        }
      ...
      if (m_BlockHeader.Type == NHeader::NBlockType::kFileHeader
          #ifdef __OS2__ 
          || strcmp(item.Name, kSubHeadTypeOS2EA) == 0
          #endif
          )
      {
        return S_OK;
