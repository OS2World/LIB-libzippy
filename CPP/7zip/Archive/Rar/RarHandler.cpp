...
static const STATPROPSTG kProps[] =
{
  ...
  { NULL, kpidOS2EA, VT_BSTR},
  { NULL, kpidOS2EASize, VT_UI4}
};
...
STDMETHODIMP CHandler::GetProperty(UInt32 index, PROPID propID,  PROPVARIANT *value)
{
  ...
  switch(propID)
  {
    ...
    #ifdef __OS2__
    case kpidOS2EASize:
      for (int i = 0; i < item.SubBlocks.Size(); i++) {
        const CItemEx &sb = item.SubBlocks[i];
        if (strcmp(sb.Name, kSubHeadTypeOS2EA) == 0) {
          prop = (UInt32)sb.Size;
          break;
        }
      }
    ...
    case kpidOS2EA:
      // For encrypted archives this is valid only after caching of the
      // encryption password during extracting of any archive item.
      for (int i = 0; i < item.SubBlocks.Size(); i++) {
        const CItemEx &sb = item.SubBlocks[i];
        if (strcmp(sb.Name, kSubHeadTypeOS2EA) == 0 && !sb.IsSplitBefore()) {
          prop.bstrVal = SysAllocStringByteLen(0, sb.Size);
          if (prop.bstrVal != NULL) {
            if(SubBlockExtract((Byte*)prop.bstrVal, sb.Size, item, i) == S_OK) {
              prop.vt = VT_BSTR;
            } else {
...
{
  {
    ...
    for (;;)
    {
      ...
      for (;;)
      {
        ...
        if (item.Type == NHeader::NBlockType::kSubBlock) {
          if(!_items.IsEmpty()) {
            _items.Back().SubBlocks.Add(item);
          }
          continue;
        }
...
#ifdef __OS2__
STDMETHODIMP CHandler::SubBlockExtract(Byte *target, UInt32 size, const CItemEx &item, int index)
{
  static CObjectVector<CMethodItem> methodItems;
  CMyComPtr<ICompressCoder> commonCoder;
  const CItemEx& sb = item.SubBlocks[index];

  CByteBuffer pData(sb.SubBlockData.GetCapacity());
  memcpy((Byte*)pData, (const Byte*)sb.SubBlockData, sb.SubBlockData.GetCapacity());
  UInt32 CRC = sb.FileCRC;

  if (sb.IsSplitAfter()) {
    for (int i = index + 1; i < item.SubBlocks.Size(); i++) {
      if (strcmp(item.SubBlocks[i].Name, sb.Name) == 0) {
        if (item.SubBlocks[i].IsSplitBefore()) 
        {
          UInt32 pos = pData.GetCapacity();
          UInt32 add_size = item.SubBlocks[i].SubBlockData.GetCapacity();

          pData.SetCapacity(pData.GetCapacity() + add_size);
          memcpy((Byte*)pData + pos, (const Byte*)item.SubBlocks[i].SubBlockData, add_size);
          CRC = item.SubBlocks[i].FileCRC;
        } else {
          break;
        }
      }
  ...
  if (sb.IsEncrypted())  {
    if (sb.UnPackVersion >= 29) {
      NCrypto::NRar29::CDecoder rar29CryptoDecoder;
      rar29CryptoDecoder.SetRar350Mode(sb.UnPackVersion < 36);
      RINOK(rar29CryptoDecoder.CryptoSetPassword((const Byte *)(const wchar_t*)_cachedPassword,
                                                  _cachedPassword.Length()*2));
      if (sb.HasSalt()) {
        RINOK(rar29CryptoDecoder.SetDecoderProperties2(sb.Salt, sizeof(sb.Salt)));
      }
      rar29CryptoDecoder.Init();
      rar29CryptoDecoder.Filter((Byte*)pData, pData.GetCapacity());
    } else if (sb.UnPackVersion >= 20) {
      NCrypto::NRar20::CDecoder rar20CryptoDecoder;
      AString oemPassword = UnicodeStringToMultiByte((const wchar_t *)_cachedPassword, CP_OEMCP);
      RINOK(rar20CryptoDecoder.CryptoSetPassword((const Byte *)(const char *)oemPassword,
                                                  oemPassword.Length()));
      rar20CryptoDecoder.Init();
      rar20CryptoDecoder.Filter((Byte*)pData, pData.GetCapacity());
    }
    else {
  ...
  switch(sb.Method) {
    case '0': {
      commonCoder = new NCompress::CCopyCoder;
      break;
    }
    case '1':
    case '2':
    ...
    case '5':
    {
      int m;
      for (m = 0; m < methodItems.Size(); m++)
        if (methodItems[m].RarUnPackVersion == sb.UnPackVersion)
          break;

      if (m == methodItems.Size())
      {
        CMethodItem mi;
        mi.RarUnPackVersion = sb.UnPackVersion;
        mi.Coder.Release();

        if (sb.UnPackVersion <= 30) {
          UInt32 methodID = 0x040300;
          if (sb.UnPackVersion < 20) {
            methodID += 1;
          } else if (sb.UnPackVersion < 29) {
            methodID += 2;
          } else {
            methodID += 3;
          }
  ...
  CSeqInMemStream inMem((const Byte*)pData, pData.GetCapacity());
  CSeqOutMemStream outMem(target, size);

  UInt64 src_size = pData.GetCapacity();
  UInt64 unp_size = size;

  RINOK(commonCoder->Code(&inMem, &outMem, &src_size, &unp_size, NULL));

  if (outMem.GetCRC() != CRC) {
