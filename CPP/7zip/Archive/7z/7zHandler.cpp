...
STDMETHODIMP CHandler::GetProperty(UInt32 index, PROPID propID,  PROPVARIANT *value)
{
  ...
  */
  ...
  switch(propID)
  {
    ...
    case kpidOS2EASize:
      if (item.EAsDefined) {
        prop = (UInt32)item.EAs.GetCapacity();
      }
      break;

    case kpidOS2EA:
      if (item.EAsDefined) {
        prop.bstrVal = SysAllocStringByteLen(0, item.EAs.GetCapacity());
        if (prop.bstrVal != NULL) {
          memcpy((Byte*)prop.bstrVal, (const Byte*)item.EAs, item.EAs.GetCapacity());
          prop.vt = VT_BSTR;
        } else {
          res = E_OUTOFMEMORY;
        }
