    ...
    IArchiveUpdateCallback *updateCallback)
{
  ...
  for (UInt32 i = 0; i < numItems; i++)
  {
    ...
    if (ui.NewProps)
    {
      ...
      {
        NCOM::CPropVariant prop;
        RINOK(updateCallback->GetProperty(i, kpidOS2EA, &prop));
        if (prop.vt == VT_EMPTY)
          ui.EAsDefined = false;
        else if (prop.vt != VT_BSTR)
          return E_INVALIDARG;
        else {
          ui.EAs.SetCapacity(SysStringByteLen(prop.bstrVal));
          memcpy((Byte*)ui.EAs, prop.bstrVal, ui.EAs.GetCapacity());
          ui.EAsDefined = true;
        }
      }
      #endif
