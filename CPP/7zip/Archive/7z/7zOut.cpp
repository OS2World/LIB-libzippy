    ...
    UInt64 &headerOffset)
{
  ...
  #ifdef __OS2__
  {
    /* ---------- Write OS/2 Extended Attributes ---------- */
    int numDefined = 0;
    size_t EAsDataSize = 0;

    for (int i = 0; i < db.Files.Size(); i++) {
      if (db.Files[i].EAsDefined) {
        numDefined++;
        EAsDataSize += db.Files[i].EAs.GetCapacity();
      }
      EAsDataSize += sizeof(UInt32);
    }
    if (numDefined > 0)
    {
      EAsDataSize++;
      SkipAlign(2 + GetBigNumberSize(EAsDataSize), 2);

      WriteByte(NID::kOS2EA);
      WriteNumber(EAsDataSize);
      WriteByte(0);
      for (int i = 0; i < db.Files.Size(); i++) {
        if (db.Files[i].EAsDefined) {
          WriteUInt32(db.Files[i].EAs.GetCapacity());
          WriteBytes((const Byte*)db.Files[i].EAs, db.Files[i].EAs.GetCapacity());
        } else {
          WriteUInt32(0);
        }
      }
