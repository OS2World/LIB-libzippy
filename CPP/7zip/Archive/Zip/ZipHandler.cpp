  ...
  switch(propID)
  {
    ...
    case kpidOS2EASize:
      for (int i = 0; i < item.CentralExtra.SubBlocks.Size(); i++) {
        const CExtraSubBlock &sb = item.CentralExtra.SubBlocks[i];
        if (sb.ID == NFileHeader::NExtraID::kOS2EA) {
          prop = GetUi32((const Byte*)sb.Data);
          break;
        }
      }
      break;
    ...
    case kpidOS2EA:
      for (int i = 0; i < item.LocalExtra.SubBlocks.Size(); i++) {
        const CExtraSubBlock &sb = item.LocalExtra.SubBlocks[i];
        if (sb.ID == NFileHeader::NExtraID::kOS2EA) {
          prop.bstrVal = SysAllocStringByteLen(0, GetUi32((const Byte*)sb.Data));
          if (prop.bstrVal != NULL) {
            if(MemExtract((Byte*)prop.bstrVal, GetUi32((const Byte*)sb.Data), 
                          ((const Byte*)sb.Data)+4, sb.Data.GetCapacity()-4) == S_OK)
            {
              prop.vt = VT_BSTR;
            } else {
...
#ifdef __OS2__
STDMETHODIMP CHandler::MemExtract(Byte *target, UInt32 tgt_size, 
                                  const Byte *source, UInt32 src_size)
{
  static CObjectVector<CMethodItem> methodItems;
  UInt16 methodId = GetUi16(source);
  UInt32 extra_field_crc = GetUi32(source+2);
  src_size -= 6;
  ...
  CSeqInMemStream inMem(source+6, src_size);
  CSeqOutMemStream outMem(target, tgt_size);
  UInt64 res_size = tgt_size;

  HRESULT result = coder->Code(&inMem, &outMem, NULL, &res_size, NULL);

  if(outMem.GetCRC() != extra_field_crc) {
